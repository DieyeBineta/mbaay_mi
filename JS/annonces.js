...
const categories = [...new Set(annonces.map(a => a.titre))];
const zones = [...new Set(annonces.map(a => a.lieu))];
const prix = [...new Set(annonces.map(a => a.prix))];
...
prix.forEach(p => {
  ...
  selectPrix.appendChild(option);
});

const boutonRecherche = document.querySelector(".select-btn");

boutonRecherche.addEventListener("click", () => {
  const catValue = selectCategorie.value;
  const zoneValue = selectZone.value;
  const prixValue = selectPrix.value;

  
  const annoncesFiltres = annonces.filter(a => {
    return (catValue === "" || a.titre === catValue) &&
           (zoneValue === "" || a.lieu === zoneValue) &&
           (prixValue === "" || a.prix === prixValue);
  });
